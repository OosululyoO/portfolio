...
const BackToHomeButton = () => {
  ...
  return (
    ...
    >
      <Link 
        to="/" 
        className="w-14 h-14 bg-blue-600/40 backdrop-blur-md shadow-lg rounded-full flex items-center justify-center text-white border border-white/20"
      >
...
const CategoryPage = ({ data, onOpenModal }) => {
  ...
  return (
    <main className="pt-40 pb-20 px-10 md:px-24 min-h-[70vh]">
      <motion.h1 className="text-6xl md:text-8xl font-black mb-16 tracking-tighter bg-gradient-to-r from-blue-600 to-cyan-500 bg-clip-text text-transparent uppercase">
        {categoryData.title}
      </motion.h1>
...
export default function App() {
  ...
  useEffect(() => {
    const loadAll = async () => {
      ...
      if (heroKey) setHero(yaml.load(heroModules[heroKey]));
