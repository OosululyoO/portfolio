...
const DetailModal = ({ isOpen, onClose, content }) => {
              ...
              <header className="mb-16">
                <div className="flex flex-wrap items-center gap-3 mb-6">
                  {categories?.map((cat) => (
                    <span
                      key={cat}
                      className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-600 bg-blue-50 px-3 py-1 rounded-full border border-blue-100"
                    >
                      {cat}
                    </span>
                  ))}
                  {date && (
                    <span className="text-[10px] font-bold text-slate-400 ml-auto tracking-widest uppercase italic">
                      {date}
                    </span>
                  )}
                </div>
                <h2 className="text-5xl md:text-7xl font-black tracking-tighter text-slate-900 mb-8 uppercase leading-[0.9]">
                  {title}
                </h2>
  ...
  prose-p:leading-relaxed whitespace-pre-wrap"
              ...
              {extra_images?.length > 0 && (
                <div className="mt-24">
                  <h3 className="text-sm font-black uppercase tracking-[0.4em] text-slate-300 mb-10 text-center">
                    Gallery
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {extra_images.map((img, index) => (
                      <div
                        key={index}
                        className="rounded-[2rem] overflow-hidden border border-slate-100 shadow-lg"
                      >
                        <img
                          src={getAssetPath(img)}
